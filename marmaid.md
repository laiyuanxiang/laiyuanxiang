...
Mindloom 的核心概念包括 **模板（Template）**、**语义组件（Semantic Component）**、**单元（Unit）**、**节点（Node）**、**智能体（Agent）**、**调用（Call）**、**数据域（Data Scope）** 和 **参数类型（Parameter Type）**。这些概念共同构成了 Mindloom 的基本架构，确保了系统执行逻辑的组织、执行和控制。以下是对这些概念的详细解释。
...
**语义组件** 是 Mindloom 系统中的重要构成单元，它包括调度器（Scheduler）和执行器（Executor）两个大类。每个语义组件负责定义和控制系统中的特定任务或行为。具体的语义组件包括任务（Task）、流程（Process）、AI 生成器Generator）、工具（Tool）和操作（Action），系统中的每个单元、节点都是某种语义组件的具体体现。它们共同构建了一个灵活且可控的执行框架。
...
* **任务（Task）**：任务是执行的最外层单元，代表一个完整的 Agent 生命周期。任务定义了执行的入口和出口，并通过 `调用` 触发后续的流程。
* **流程（Process）**：流程是 Agent 运行流程控制的核心，定义了任务的执行顺序、分支、循环、并行等控制逻辑。
...
* **AI 生成器（Generator）**：AI 生成器单元负责与 AI 模型（如 LLM）交互，通过输入提示生成内容，并返回生成结果。
* **工具（Tool）**：工具单元提供引擎内置的基本功能，如计算、数据处理等。
* **操作（Action）**：操作单元负责与外部系统进行交互，调用外部服务或执行外部接口的操作。
...
**目标**：明确 mindloom 系统的核心公理和原则，构建清晰的语义框架。
...
## 3.3 call 为唯一执行跃迁机制
call 作为唯一的执行跃迁机制，是 mindloom 中从一个执行状态到另一个执行状态的桥梁。每次执行的最小单元是 call，它负责触发执行过程并传递控制流。所有其他的执行动作、工具调用、操作和AI生成都依赖于 call 的机制，确保了系统的递归计算和一致性。
