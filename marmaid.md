...
Mindloom 的核心概念包括 **模板（Template）**、**语义组件**、**单元（Unit）**、**节点（Node）**、**智能体（Agent）**、**调用（Call）**、**数据域（Data Scope）** 和 **参数类型（Parameter Type）**。这些概念共同构成了 Mindloom 的基本架构，确保了系统执行逻辑的组织、执行和控制。以下是对这些概念的详细解释。
