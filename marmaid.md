...
* **并行执行**：虽然 Tool 和 Action 本身不支持并行执行，但可以在 **Process** 节点中定义多个并行的 `调用`，并行执行不同的工具和操作。
* **错误处理**：在与外部系统交互时，错误的处理方式由模板预先定义，并根据执行的 `调用` 的输出映射返回错误信息。
...
2. **调用机制**：所有外部工具的使用都通过 `调用` 实现。`调用` 负责传递参数并触发目标工具的执行。
